...
#----------------------------------------------------------------------------------#
# Globals
my ($instance,$meta_schemas,$meta_tables,$userid,$password,$company_name,$verbose,$contributors,$version_display,$debug,$excel,$dblink,$csv,$csvheader,$csvseparator,$csvlinefeed,$author,$gnu,$quiet,$help,$man,$no_truncate);
my $sql;                            # SQL String
my @row;                            # Result set
...
sub version_display{
  print '$Header: ScriptData.pl 1.6 2005/09/12 14:17:42BST ghoekstra DEV  $'."\n";
  exit(0);
}
...
[-l|--dblink DBLink] [-s|--schemas Schemas] [-o|--objects Tables] [--ignore-empty] [--no-truncate]
...
Do not include a table truncation command before inserting the data.

=item B<--legacy>

Make the script Oracle 8i-compatible.
...
to create the objects on an Oracle instance:
    ...
    $ sqlplus [schema]/[password]@[instance] @[script]
