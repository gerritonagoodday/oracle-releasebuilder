...
#----------------------------------------------------------------------------------#
# FUNCTION:
# Scripts the creation scripts of Oracle views into nice installation files.
#----------------------------------------------------------------------------------#
...
sub version_display{
  print '$Header: ScriptViews.pl 1.3 2005/09/09 13:55:12BST ghoekstra DEV  $'."\n";
  exit(0);
}
...
sub get_object_details{
  ...
  my $sql="select text_length, text ".
          "  from sys.all_views".(defined($dblink)?'\@'.$dblink:'').
          " where owner = ".$dbh->quote($owner).
          "   and view_name = ".$dbh->quote(uc($object));
...
#----------------------------------------------------------------------------------#
# Get all details for type
sub get_object_columns{
  my ($connection,$cursor,$owner,$object)=@_;
  my $sql="select *".
          "  from sys.all_tab_cols".(defined($dblink)?'\@'.$dblink:'').
          " where owner = ".$dbh->quote(uc($owner)).
          "   and table_name = ".$dbh->quote(uc($object)).
          " order by column_id";
  $cursor=$connection->prepare($sql);
  $cursor->execute() || die "Error getting columns of $owner.$object".(defined($dblink)?'\@'.$dblink:'')."\n".$cursor->errstr()."\n";
  return $cursor;
}
...
sub does_object_exist{
  my ($connection,$cursor,$owner,$object)=@_;
  my $sql="select distinct owner, view_name ".
          "  from sys.all_views".(defined($dblink)?'\@'.$dblink:'').
          " where owner=upper(".$dbh->quote($owner).") ".
          "   and view_name=upper(".$dbh->quote($object).") ".
          " order by 1,2";
  $cursor=$connection->prepare($sql);
...
sub ScriptTextFile{
  ...
  foreach (@schema_objects){
    ...
    my $file_name = lc($owner).".views.".lc($object).".sql";
    Oracle::Script::Common->info("Building creation script for view $owner.$object".(defined($dblink)?'@'.$dblink:'')." in file $short_file_name\n") if(!$quiet);
    ...
    print F "-- Creation script for view $owner.$object\n--\n";
    ...
    print F "prompt Creating view $owner.$object\n";
    print F "\n";
    if($legacy){
    print F "-- Drop view if it already exists\n";
    print F "-- Note that referential constraints will also be dropped.\n";
    ...
    print F "   where object_type = 'VIEW'\n";
    print F "     and owner = upper('".$owner."')\n";
    print F "     and object_name = upper('".$object."');\n";
    print F "  if(v_count>0)then\n";
    print F "    execute immediate 'drop view $owner.$object';\n";
    print F "  end if;\n";
    print F "exception\n";
    print F "  when others then\n";
    print F "    if(v_count>0)then\n";
    print F "      dbms_output.put_line('WARNING: Dropping referential constraints to $owner.$object');\n";
    print F "      execute immediate 'drop view $owner.$object cascade constraints';\n";
    print F "    end if;\n";
    print F "end;\n";
    print F "/\n";
    }
    ...
    print F "-- Create view\n";
    print F "-- \n";
    print F "-- Note: If this view does not build then it may be because the objects\n";
    print F "--       referred to are not fully qualified. The best place to fix is in the\n";
    print F "--       source database and to regenerate this script.\n";
    print F "------------------------------------------------------------------------------\n";
    print F "\n";
    if($legacy){
    print F "create view $owner.$object (\n";
    }else{
    print F "create or replace view $owner.$object (\n";
    }
    # Get columns
    my $c_object_colums;
    $c_object_colums=get_object_columns($dbh,$c_object_colums,$owner,$object);
    my $one_shot=0;
    my $r_object_colums;
    while($r_object_colums=$c_object_colums->fetchrow_hashref()){
      print F ($one_shot==0?' ':',')." $r_object_colums->{COLUMN_NAME}\n";
      $one_shot=1;
    }
    print F ") as \n";
    # Get text
    my $c_object_text;
    $c_object_text=get_object_details($dbh,$c_object_text,$owner,$object);
    $one_shot=0;
    my $r_object_text;
    while($r_object_text=$c_object_text->fetchrow_hashref()){
      print F "$r_object_text->{TEXT}"; # Should only be one record...
    }
    print F ";\n";
    print F "/\n";
    print F "\n";
...
ScriptViews.pl - Generates formal creation scripts for Oracle Views

=head1 SYNOPSIS

=over 4

=item B<ScriptViews.pl> B<-i> db1 B<-u> user B<-p> passwd

Builds the creation scripts for all the views in the schema 'user' on
database instance db1.

=item B<ScriptViews.pl> B<-i> db1 B<-u> user B<-p> passswd B<-m> schema1 B<-m> schema2

Builds the creation scripts only for all the views in the schemas 'schema1'
...
=item B<ScriptViews.pl> B<-i> db1 B<-u> user B<-p> passwd B<-t> schema1.table1

Builds the creation scripts only for the view 'schema1.table1' on database
instance db1.

=item B<ScriptViews.pl> B<-i> db1 B<-u> user B<-p> passwd -l db2

Builds the creation scripts for all the users on the database referred to over
the database link db2.

=item B<ScriptViews.pl> B<--help>
...
B<ScriptViews.pl> B<-u|--user> UserId B<-p|--password> Passwd [B<-i|--instance> Instance name]
[-l|--dblink DBLink] [-s|--schemas Schemas] [-o|--objects Views] [--no-storage] [--legacy]
