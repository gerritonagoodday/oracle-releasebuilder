...
my ($instance,$meta_schemas,@meta_schemas,$meta_tables,@objects,$userid,$password,$author,$verbose,$contributors,$version_display,$debug,$excel,$dblink,$quiet,$man,$help,$gnu,$grants,$no_storage,$legacy);

#----------------------------------------------------------------------------------#
# Prints the version of this script and exits the program
sub version_display{
  print '$Header: ScriptTables.pl 1.15 2005/09/12 14:17:43BST ghoekstra DEV  $'."\n";
  exit(0);
}
...
# The partitions are ordered by partition_name although the name does not necessarily
# indicate the correct order when RANGE partitions are used.
sub get_table_partitions{
  ...
  my $sql=
      ...
      "   and rtrim(c.object_type) = 'TABLE'
          and a.table_owner=t.owner
          and a.table_name=t.table_name
          and a.partition_name = nvl(p.partition_name,s.partition_name)
        order by a.partition_position asc, nvl(p.partition_name,s.partition_name) asc";
  $cursor=$connection->prepare($sql);
...
sub ScriptSpreadsheet{
  ...
  foreach (@schema_tables){
    ...
    while($tc_row_ref=$c_table->fetchrow_hashref()){
      ...
      # Precision
      if($tc_row_ref->{DATA_TYPE} eq 'NUMBER'){
        if(defined $tc_row_ref->{DATA_PRECISION}){
          if($tc_row_ref->{DATA_PRECISION}>0){
        ...
        if($tc_row_ref->{DATA_TYPE}=~/DATE|LOB|RAW|LONG|BFILE|ROWID|XML|TIMESTAMP/){
          if(defined $tc_row_ref->{DATA_PRECISION} && $tc_row_ref->{DATA_PRECISION}>0){
            if(defined $tc_row_ref->{DATA_SCALE} && $tc_row_ref->{DATA_SCALE}>0){
              $sql.="($tc_row_ref->{DATA_PRECISION},$tc_row_ref->{DATA_SCALE})";
            }else{
              # Data scale is 0 - ignore
              $sql.="($tc_row_ref->{DATA_PRECISION})";
            }
          }
        }else{
...
sub ScriptTextFile{
  ...
  foreach (@schema_tables){
    ...
    while($colum_row_ref=$c_columns->fetchrow_hashref()){
      ...
      # Precision
      if($colum_row_ref->{DATA_TYPE} eq 'NUMBER'){
        if(defined $colum_row_ref->{DATA_PRECISION}){
          if($colum_row_ref->{DATA_PRECISION}>0){
      ...
      $long=~s/\n$//; # partykeer is daar 'n einde-van-lyn karakter hier - verwyder dit!
...
[-l|--dblink DBLink] [-s|--schemas Schemas] [-o|--objects Tables] [--no-storage] [--legacy]
[-g|--grants] [--ignore-empty] [-x|--spreadsheet] [--contributors] [-v|--version]
...
Make the script Oracle 8i-compatible. In this case, many objects cannot
be created using the create-or-replace syntax, and need to manually be
removed before the object is recreated.
