...
sub version_display{
  print '$Header: ScriptUsers.pl 1.1 2005/08/24 10:54:06BST ghoekstra DEV  $'."\n";
  exit(0);
}
...
sub get_scriptable_objects{
  ...
  my $sql=
      ...
      " where username not like ".$dbh->quote('SYS%').
      "   and username not like ".$dbh->quote('OPS$%').
      "   and username <> ".$dbh->quote('OUTLN').
      " order by 1";
...
sub get_object_details{
  ...
  my $sql=
      "select * ".
      "  from sys.dba_users".(defined($dblink)?'\@'.$dblink:'').
      " where username=upper(".$connection->quote($object).") ";
  $cursor=$connection->prepare($sql);
...
sub ScriptTextFile{
  ...
  foreach (@schema_objects){
    ...
    $c_object=get_object_details($dbh, $c_object, $owner, $object);
    my $object_row_ref=$c_object->fetchrow_hashref();
    die "Failed to retrieve details of $owner.$object".(defined($dblink)?'@'.$dblink:'') if(!defined $object_row_ref->{USERNAME});
    my $total_cols=0;

    # Make up file name
    my $short_file_name=lc($object).".sql";
    my $file_name = lc($owner).".users.".lc($object).".sql";
    Oracle::Script::Common->info("Building creation script for user $owner.$object".(defined($dblink)?'@'.$dblink:'')." in file $short_file_name\n") if(!$quiet);
    ...
    print F "prompt Creating user $object\n";
    print F "\n";
    print F "-- Drop user if already exists\n";
    print F "-- Note that the database objects dependent on this object will also be dropped.\n";
    print F "declare \n";
    ...
    print F "   where user = upper('".$object."');\n";
    print F "  if(v_count>0)then\n";
    print F "    execute immediate 'drop user $object cascade';\n";
    print F "  end if;\n";
    print F "exception\n";
    print F "  when others then\n";
    print F "    dbms_output.put_line('WARNING: Could not drop user $object. '||sqlerrm);\n";
    print F "end;\n";
    print F "/\n";
    ...
    while(my $object_script_ref=$c_object->fetchrow_hashref()){
      print   F "create user $object\n";
      print   F "  identified by values '$object_script_ref->{PASSWORD}'\n";
      print   F "  default tablespace $object_script_ref->{DEFAULT_TABLESPACE}\n";
      print   F "  temporary tablespace $object_script_ref->{TEMPORARY_TABLESPACE}\n";
      print   F "  profile $object_script_ref->{PROFILE}\n";
      my $c_tablespaces;
      $c_tablespaces=get_tablespace_details($dbh,$c_tablespaces,$owner,$object);
      while(my $tablespace_script_ref=$c_tablespaces->fetchrow_hashref()){
        print F "  quota ",$tablespace_script_ref->{MAX_BYTES}==-1?"unlimited":$tablespace_script_ref->{MAX_BYTES}," on $tablespace_script_ref->{TABLESPACE_NAME}\n";
      }
      print   F ";\n";
    }
...
ScriptUsers.pl - Generates formal creation scripts for Oracle users (a.k.a. schemas)

=head1 SYNOPSIS

=over 4

=item B<ScriptUsers.pl> B<-i> db1 B<-u> user B<-p> passwd

Builds the creation scripts for all the Oracle users (a.k.a schemas) on the
database instance db1.

=item B<ScriptUsers.pl> B<-i> db1 B<-u> user B<-p> passswd B<-o> user_name

Builds the creation script for the indicated user name only on database instance db1.

=item B<ScriptUsers.pl> B<-i> db1 B<-u> user B<-p> passwd B<-l> db2
...
B<ScriptUsers.pl> B<-u|--user> UserId B<-p|--password> Passwd [B<-i|--instance> Instance name]
