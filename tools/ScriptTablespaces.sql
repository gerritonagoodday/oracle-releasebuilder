...
--  Header: $
------------------------------------------------------------------------------
-- ScriptTableSpaces.sql Version 0.5
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
-- Scripts the creation scripts for all the table spaces
-- in the currently logged-in Oracle instance, A single file is generated,
-- called TablesSpaces.sql
-- The output files can be used to create an installation of an Oracle 8i
-- 9i and 10g databases.
--
-- The output file will appear in the utl_file directory, if it has been defined.
-- Note that existing files will be overwritten.
--
-- Usage:
-- ~~~~~
-- sqlplus -s "sys/[password]@[instance] as sysdba" @ScriptTableSpaces.sql tablespaces
--   where tablespaces is a comma-delimited string of table spaces for which scripts
--   need to be created for, e.g. billy_space,bubba_space,bobby_space.
--
-- Notes:
-- ~~~~~
...
-- Outstanding Issues:
-- ~~~~~~~~~~~~~~~~~~
-- * Table spaces are implemented depending on the hardware configuration
--   of the target hardware using forward slashes, backward slashes etc..
------------------------------------------------------------------------------
...
prompt Enter the comma-delimited names of the tablespaces in the &db_name database for which
prompt Tablespace creation scripts need to be created for, e.g. UTL_DB,CRD_DB,DBT_DB. Or nothing to abort.
variable tablespaces varchar2(4000);
declare
...
begin
  ...
  if(:tablespaces is null)then
    dbms_output.put_line('Tablespace name not specified. Exiting...');
    raise ex_no_input;
  else
    v_message:='Building Tablespace creation scripts used by schemas ['||:tablespaces||']';
...
declare
  ...
  cursor c_schemas is
    select distinct
     ...
     where instr(upper(:schemas),?) <> 0

  -- Get all table space names that match the criterias
  cursor c_tablespaces is
    select *
      from sys.dba_tablespaces
     where instr(upper(:tablespaces),tablespace_name) <> 0
     order by tablespace_name;

  -- Get actual files names and paths
  select *
    from ku$_tablespace_view t


begin
  v_start:=dbms_utility.get_time;

  dbms_output.put_line('Building create script for tablespaces referenced by the following schemas:');
  for c_s in c_schemas loop
    dbms_output.put_line(c_s.?);
  end loop;


  for c_s in c_tablespaces loop
    dbms_output.put_line(c_s.tablespace_name);
  end loop;
  -- Make up file name
  v_file_name := 'TableSpaces.sql';
  ...
  utl_file.put_line(v_file_handle, '--  Tablespace creation file for database %ORACLE_SID% on %ORACLE_HOST%.');
  utl_file.put_line(v_file_handle, '--');
  utl_file.put_line(v_file_handle, '--  USAGE:');
  utl_file.put_line(v_file_handle, '--  $ export ORACLE_SID=%ORACLE_SID%');
  utl_file.put_line(v_file_handle, '--  $ sqlplus "sys/<password> as sysdba" install/@%ORACLE_SID%_tablespaces.sql');
  ...
  utl_file.put_line(v_file_handle, '--  This file was originally generated by the ScriptTableSpaces.sql utility');
  ...
  utl_file.put_line(v_file_handle, 'spool log/%ORACLE_SID%_tablespaces.log');
  ...
  for c in c_tablespaces loop
    utl_file.put_line(v_file_handle, '-- Only create tablespace '||c.tablespace_name||' if it does not exist:');
    utl_file.put_line(v_file_handle, 'declare ');
    utl_file.put_line(v_file_handle, '  v_count integer;');
    utl_file.put_line(v_file_handle, '  v_db_name varchar2(100);');
    ...
    utl_file.put_line(v_file_handle, '    from sys.dba_tablespaces');
    utl_file.put_line(v_file_handle, '   where tablespace_name = '''||c.tablespace_name||''';');
    utl_file.put_line(v_file_handle, '  if(v_count=0) then');
    utl_file.put_line(v_file_handle, '    -- Get current Instance name');
    utl_file.put_line(v_file_handle, '    select sys_context(''userenv'',''db_name'') into v_db_name from dual;');
    utl_file.put_line(v_file_handle, '    dbms_output.put_line(''Creating table space '||c.tablespace_name||''');');
    utl_file.put_line(v_file_handle, '    if(instr(dbms_utility.port_string(),''WIN'')>0)then');
    utl_file.put_line(v_file_handle, '      execute immediate ''create tablespace "'||c.tablespace_name||'" ''||');
    utl_file.put_line(v_file_handle, '        '''||c.logging||' ''||');
    utl_file.put_line(v_file_handle, '        ''DATAFILE ''''%ORACLE_BASE%\ORADATA\''||v_db_name||''\'||c.tablespace_name||'.dbf'''' SIZE 100M ''||');
    utl_file.put_line(v_file_handle, '        ''AUTOEXTEND ON NEXT 10M MAXSIZE UNLIMITED EXTENT MANAGEMENT LOCAL'';');
    ...
    utl_file.put_line(v_file_handle, '  else');
    utl_file.put_line(v_file_handle, '    dbms_output.put_line(''Table space '||c.tablespace_name||' already exists'');');
    utl_file.put_line(v_file_handle, '  end if;');
    utl_file.put_line(v_file_handle, 'exception');
    utl_file.put_line(v_file_handle, '  when others then');
    utl_file.put_line(v_file_handle, '    dbms_output.put_line(''* Exception: Error Code: [''||to_char(sqlcode)||''] Message: [''||sqlerrm||''].'');');
