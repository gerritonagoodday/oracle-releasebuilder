...
=item C<get_dblink_full_name(database connection handle,database link name)>
...
=item C<get_nls_format(database connection handle)>

Gets the NLS date format string
...
sub get_nls_format {
  my $self=shift;
  my $connection=shift;
  my $sql="select sys_context(".$connection->quote('userenv').",".$connection->quote('NLS_DATE_FORMAT').") from dual";
  my $cursor=$connection->prepare($sql);
  $cursor->execute() || warn "Could not get NLS_DATE_FORMAT.\n";
  return $cursor->{1};
}
...
who modified this:
    ...
    $Header: Common.pm 1.1 2005/01/31 09:26:41GMT ghoekstra DEV  $
