...
#----------------------------------------------------------------------------------#
# FUNCTION:
# Scripts the creation scripts of Oracle triggers into nice installation files.
#----------------------------------------------------------------------------------#
...
sub version_display{
  print '$Header: ScriptTriggers.pl 1.1 2005/01/31 09:29:58GMT ghoekstra DEV  $'."\n";
  exit(0);
}
...
sub get_object_details{
  ...
  my $sql="select *".
          "  from sys.all_triggers".(defined($dblink)?'\@'.$dblink:'').
          " where owner = ".$dbh->quote($owner).
          "   and trigger_name = ".$dbh->quote(uc($object));
...
sub does_object_exist{
  my ($connection,$cursor,$owner,$object)=@_;
  my $sql="select distinct owner, trigger_name ".
          "  from sys.all_triggers".(defined($dblink)?'\@'.$dblink:'').
          " where owner=upper(".$dbh->quote($owner).") ".
          "   and trigger_name=upper(".$dbh->quote($object).") ".
          " order by 1,2";
  $cursor=$connection->prepare($sql);
...
sub ScriptTextFile{
  ...
  foreach (@schema_objects){
    ...
    my $file_name = lc($owner).".triggers.".lc($object).".sql";
    Oracle::Script::Common->info("Building creation script for trigger $owner.$object".(defined($dblink)?'@'.$dblink:'')." in file $short_file_name\n") if(!$quiet);
    ...
    print F "-- Creation script for trigger $owner.$object\n--\n";
    ...
    print F "prompt Creating trigger $owner.$object\n";
    print F "\n";
    print F "-- Drop trigger if it already exists\n";
    print F "declare \n";
    ...
    print F "create trigger $owner.$object\n";
    # Get object details
    my $c_object_details;
    $c_object_details=get_object_details($dbh,$c_object_details,$owner,$object);
    my $r_object_details = $c_object_details->fetchrow_hashref();
    print F lc("  $r_object_details->{TRIGGER_TYPE} $r_object_details->{TRIGGERING_EVENT} on ");
    print F "$r_object_details->{TABLE_OWNER}.$r_object_details->{TABLE_NAME}\n";
    print F lc("  $r_object_details->{REFERENCING_NAMES}\n");
    if($r_object_details->{DESCRIPTION}=~m/FOR EACH ROW/ig){
      print F "  for each row\n";
    }
    # Preserve comments
    my @lines=split("\n",$r_object_details->{DESCRIPTION});
    foreach (@lines){
      print F "$_\n" if m/^ *--+/ && !m/^ *-* *\$/;
    }
    # Body
    print F "$r_object_details->{TRIGGER_BODY}";
    print F "\n";
...
if($meta_types){
  ...
  foreach my $object(@spec_objects){
    ...
    if(scalar(@row)==0){
      die "Individually specified object ",uc(@schema_object[0].@schema_object[1])," does not exist.\n";
    }else{
...
ScriptTriggers.pl - Generates formal creation scripts for Oracle data types

=head1 SYNOPSIS

=over 4

=item B<ScriptTriggers.pl> B<-i> db1 B<-u> user B<-p> passwd

Builds the creation scripts for all the triggers in the schema 'user' on
database instance db1.

=item B<ScriptTriggers.pl> B<-i> db1 B<-u> user B<-p> passswd B<-m> schema1 B<-m> schema2

Builds the creation scripts only for all the triggers in the schemas 'schema1'
...
=item B<ScriptTriggers.pl> B<-i> db1 B<-u> user B<-p> passwd B<-t> schema1.table1

Builds the creation scripts only for the trigger 'schema1.table1' on database
instance db1.

=item B<ScriptTriggers.pl> B<-i> db1 B<-u> user B<-p> passwd -l db2

Builds the creation scripts for all the triggers in the DBLink's default schemas
of the remote database instance, which is accessed from this database instance
db1 via database link db2.

=item B<ScriptTriggers.pl> B<-i> db1 B<-u> user B<-p> passwd B<-m> schema1,schema2 B<-l> db2

Builds the creation scripts for all the triggers in the schemas 'schema1'
and 'schema2' on a remote database instance that is accessed from this
Oracle instance db1 via the database link db2.

=item B<ScriptTriggers.pl> B<--help>
...
B<ScriptTriggers.pl> B<-u|--user> UserId B<-p|--password> Passwd [B<-i|--instance> Instance name]
[-l|--dblink DBLink] [-s|--schemas Schemas] [-o|--objects Triggers] [--no-storage]
...
=item B<-o|--objects List of triggers>
