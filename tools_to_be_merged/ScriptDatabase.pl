...
#----------------------------------------------------------------------------------#
# FUNCTION:
# Scripts a database creation script from an existing data into a installation file.
#----------------------------------------------------------------------------------#
...
print "Building creation script for database $instance".(defined($dblink)?'@'.$dblink:'')." in file $file_name\n" if(!$quiet);
...
print F "-- Creation script for database\n--\n";
...
print F "prompt Creating database\n";
print F "\n";

# TODO: Add script that checks if database already exists
...
# TODO: Script database

print F Oracle::Script::FileHeaders->MakeSQLFileFooter();
close F;

my $proc_time=time()-$start_time;
print "Created total database installation in $proc_time seconds.\n";
exit(0);
...
ScriptDatabase.pl - Scripts a database creation script from an existing data into a installation file

=head1 SYNOPSIS

=over 4

=item B<ScriptDatabase.pl> B<-i> db1 B<-u> sys_user B<-p> passswd

Builds the creation scripts of the database instance db1.

=item B<ScriptDatabase.pl> B<-i> db1 B<-u> sys_user B<-p> passswd  B<-x>

Creates a spreadsheet describing the database instance db1.

=item B<ScriptDatabase.pl> B<--help>
...
B<ScriptDatabase.pl> B<-i|--instance> DB B<-u|--user> System UserId B<-p|--password> Passwd [-l|--dblink DBLink]
...
This program creates a script based on the database instance it is pointed to
so that it can be run on a separate Oracle RDBMS installation to create an
identical database.

The resulting scripts creates only the basic database, with no application
schemas. Application schemas ans their related objects are created using
other scripting tools.

You can run the resulting script through SQLPLUS to create the desired
database on an Oracle server:

    # export ORACLE_SID=<new_database_name>
    # sqlplus /nolog
