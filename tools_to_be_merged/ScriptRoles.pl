...
$file_name="roles.sql";
Oracle::Script::Common->info("Building roles creation script for database $instance".(defined($dblink)?'@'.$dblink:'')." in file $file_name\n") if(!$quiet);
...
print F "-- Creation script for roles\n--\n";
...
print F "prompt Creating roles\n";
print F "\n";

# TODO: Add script that checks if roles already exist

print F "/\n";
print F "------------------------------------------------------------------------------\n";
print F "-- Create roles\n";
print F "------------------------------------------------------------------------------\n";
print F "\n";
...
ScriptRoles.pl - Scripts a roles creation script from an existing database into a installation file

=head1 SYNOPSIS

=over 4

=item B<ScriptRoles.pl> B<-i> db1 B<-u> sys_user B<-p> passswd

Builds the roles creation scripts of the database instance db1.

=item B<ScriptRoles.pl> B<-i> db1 B<-u> sys_user B<-p> passswd  B<-x>

Creates a spreadsheet describing the roles on database instance db1.
...
B<ScriptRoles.pl> B<-i|--instance> DB B<-u|--user> System UserId B<-p|--password> Passwd [-l|--dblink DBLink]
