...
sub version_display{
  print '$Header: ScriptViews.pl 1.1 2005/01/31 09:30:01GMT ghoekstra DEV  $'."\n";
  exit(0);
}
...
ScriptViews.pl - Generates formal creation scripts for Oracle data types
...
Builds the creation scripts for all the views in the DBLink's default schemas
of the remote database instance, which is accessed from this database instance
db1 via database link db2.

=item B<ScriptViews.pl> B<-i> db1 B<-u> user B<-p> passwd B<-m> schema1,schema2 B<-l> db2

Builds the creation scripts for all the views in the schemas 'schema1'
...
[-l|--dblink DBLink] [-s|--schemas Schemas] [-o|--objects Views] [--no-storage]
...
=item B<-o|--objects List of Views>
