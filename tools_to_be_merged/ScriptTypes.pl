...
#----------------------------------------------------------------------------------#
# FUNCTION:
# Scripts the creation scripts of Oracle type into nice installation files.
#----------------------------------------------------------------------------------#
...
sub version_display{
  print '$Header: ScriptTypes.pl 1.2 2005/03/15 18:36:56GMT ghoekstra DEV  $'."\n";
  exit(0);
}
...
sub get_object_details{
  ...
  my $sql=
      "select * ".
      "  from sys.all_types".(defined($dblink)?'\@'.$dblink:'').
      " where type_name=upper(".$connection->quote($object).") ".
...
#----------------------------------------------------------------------------------#
# Get all attributes for object
sub get_object_attributes{
  my ($connection,$cursor,$owner,$object)=@_;
  my $sql=
      "select * ".
      "  from sys.all_type_attrs".(defined($dblink)?'\@'.$dblink:'').
      " where type_name=upper(".$connection->quote($object).") ".
      "   and owner=upper(".$connection->quote($owner).") ".
      "   order by attr_no";
  $cursor=$connection->prepare($sql);
  $cursor->execute() || die "Error getting attributes of $owner.$object".(defined($dblink)?'\@'.$dblink:'')."\n".$cursor->errstr()."\n";
  return $cursor;
}

#----------------------------------------------------------------------------------#
# Get all members for object
sub get_object_members{
  my ($connection,$cursor,$owner,$object)=@_;
  my $sql=
      "select * ".
      "  from sys.all_type_methods".(defined($dblink)?'\@'.$dblink:'').
      " where type_name=upper(".$connection->quote($object).") ".
      "   and owner=upper(".$connection->quote($owner).") ".
      "   order by method_no";
  $cursor=$connection->prepare($sql);
  $cursor->execute() || die "Error getting members of $owner.$object".(defined($dblink)?'\@'.$dblink:'')."\n".$cursor->errstr()."\n";
  return $cursor;
}
...
#----------------------------------------------------------------------------------#
# Get script for type
sub get_object_script{
  my ($connection,$cursor,$owner,$object)=@_;
  my $sql=
      "select * ".
      "  from sys.all_type_versions".(defined($dblink)?'\@'.$dblink:'').
      " where type_name=upper(".$connection->quote($object).") ".
      "   and owner=upper(".$connection->quote($owner).") ".
      "   order by line";
  $cursor=$connection->prepare($sql);
  $cursor->execute() || die "Error getting script of $owner.$object".(defined($dblink)?'\@'.$dblink:'')."\n".$cursor->errstr()."\n";
  return $cursor;
}
...
sub does_object_exist{
  my ($connection,$cursor,$owner,$type)=@_;
  my $sql=
        "select distinct owner, table_name ".
        "  from sys.all_types".(defined($dblink)?'\@'.$dblink:'').
        " where owner=upper(".$dbh->quote($owner).") ".
        "   and type_name=upper(".$dbh->quote($type).") ".
        " order by 1,2";
  $cursor=$connection->prepare($sql);
  $cursor->execute() || die "Error checking if table $owner.$type exists.\n".$cursor->errstr()."\n";
  return $cursor;
}
...
sub get_scriptable_objects{
  ...
  my $sql=
      "select distinct owner, type_name".
      "  from sys.all_types".(defined($dblink)?'\@'.$dblink:'').
      " where instr(upper(".$dbh->quote($schemas)."),OWNER) <> 0".
      " order by owner,type_name";
  $cursor=$connection->prepare($sql);
  $cursor->execute() || die "Error getting all scriptable types for the string of schemas [$schemas].\n".$cursor->errstr()."\n";
  return $cursor;
}
...
sub get_metadata_ddl {
  my ($connection,$owner,$name,$type)=@_;
  my $rv;
  ...
  $cursor->bind_param(":type", uc($type));
  $cursor->bind_param(":name", uc($name));
  $cursor->bind_param(":owner",uc($owner));
  $cursor->execute || die "Error getting DDL for $type object $owner.$name.\n$DBI::errstr\n";
  $cursor->finish;
  return $rv;
}
...
sub ScriptTextFile{
  ...
  foreach (@schema_objects){
    ...
    my $file_name = lc($owner).".types.".lc($object).".sql";
    Oracle::Script::Common->info("Building creation script for type $owner.$object".(defined($dblink)?'@'.$dblink:'')." in file $short_file_name\n") if(!$quiet);
    ...
    print F "-- Creation script for type $owner.$object\n--\n";
    ...
    print F "prompt Creating type $owner.$object\n";
    print F "\n";
    print F "-- Drop type if it already exists\n";
    print F "-- Note that the database objects dependent on this type will become unusable\n";
    ...
    print F "   where object_type = 'TYPE'\n";
    print F "     and owner = upper('".$owner."')\n";
    print F "     and object_name = upper('".$object."');\n";
    print F "  if(v_count>0)then\n";
    print F "    execute immediate 'drop type $owner.$object validate';\n";
    print F "  end if;\n";
    print F "exception\n";
    print F "  when others then\n";
    print F "    if(v_count>0)then\n";
    print F "      dbms_output.put_line('WARNING: There are database objects dependent on $owner.$object');\n";
    print F "      execute immediate 'drop type $owner.$object force';\n";
    print F "    end if;\n";
    print F "end;\n";
    print F "/\n";
    ...
    my $c_object_script;
    $c_object_script=get_object_script($dbh,$c_object_script,$owner,$object);
    $one_shot=0;
    while(my $object_script_ref=$c_object_script->fetchrow_hashref()){
      $sql=$object_script_ref->{TEXT};
      if($one_shot==0){
        # First line
        # Qualify the type with the schema name
        $sql=~s/type "$object"/type $owner.$object/i;
        $sql = "create or replace $sql\n";
        # Remove extra spaces
        $sql=~s/ +/ /g;
        $one_shot=1;
      }
      # Trim and add carriage return if not present
      if(!$sql=~m/\n$/){
        $sql=~s/\s$//g;
        print F "$sql\n";
      }else{
    ...
    # Simple scripted types do not contain a trailing ';' - add it.
    if($sql=~m/[^;] *$/){
      printf F ";\n";
    }else{
      printf F "\n";
...
if(0){
    $sql.="type $owner.$object";
    my $c_object_details;
    $c_object_details = get_object_details($dbh,$c_object_details,$owner,$object);
    my $objects_ref = $c_object_details->fetchall_arrayref();
    if($objects_ref->{INCOMPLETE} eq 'NO'){
      $sql.='  as ';
      # TODO: Get invoker rights
      if($objects_ref->{TYPECODE} eq 'ANYDATA')      {
          $sql="prompt The creation of types of type $objects_ref->{TYPECODE} is not supported\n";
          warn $sql;
      }
      if($objects_ref->{TYPECODE} eq 'ANYDATASET')   {
          $sql="prompt The creation of types of type $objects_ref->{TYPECODE} is not supported\n";
          warn $sql;
      }
      if($objects_ref->{TYPECODE} eq 'ANYTYPE')      {
          $sql="prompt The creation of types of type $objects_ref->{TYPECODE} is not supported\n";
          warn $sql;
      }
      if($objects_ref->{TYPECODE} eq 'COLLECTION')   {
          $sql.="table of ";
      }
      if($objects_ref->{TYPECODE} eq 'OBJECT')       {
        $sql.="object (\n";
        # Object attributes
        my $c_object_attrs;
        $c_object_attrs=get_object_attributes($dbh,$c_object_attrs,$owner,$object);
        my $one_shot=0;
        while(my $object_attrs_ref=$c_object_attrs->fetchrow_hashref()){
          $sql.=($one_shot==0)?'    ':'   ,';
          $one_shot=1;
          $sql.="$object_attrs_ref=>{ATTR_NAME}   $object_attrs_ref=>{ATTR_TYPE_NAME}";
          if($object_attrs_ref=>{ATTR_TYPE_LENGTH}){
            $sql.="($object_attrs_ref=>{ATTR_TYPE_LENGTH})";
          }else{
            if($object_attrs_ref=>{PRECISION}){
              $sql.="($object_attrs_ref=>{PRECISION},$object_attrs_ref=>{SCALE})";
            }
          }
        }
        # Object members
        my $c_object_members;
        $c_object_members=get_object_members($dbh,$c_object_members,$owner,$object);
        while(my $object_members_ref=$c_object_members->fetchrow_hashref()){
          $sql.=($one_shot==0)?'    ':'   ,';
          $one_shot=1;
          $sql.=($object_members_ref=>{PARAMETERS}>0)?'member':'static';
          $sql.=' ';
          $sql.=($object_members_ref=>{RESULTS})?'function':'procedure';
          # etc.. It is not feasible to construct a complete type definition
          #       from the data dictionary. Use the get_object_script instead.
        }
        $sql.="  )";
      }
      if($objects_ref->{TYPECODE} eq 'XMLTYPE')      {
          $sql="prompt The creation of types of type $objects_ref->{TYPECODE} is not supported\n";
          warn $sql;
      }
      else                {
      ...
      # Final clause
      $sql.='  '.($objects_ref->{FINAL} eq 'NO')?'not ':'',"final\n";
      # Instantiation clause
      $sql.='  '.($objects_ref->{INSTANTIABLE} eq 'NO')?'not ':'',"instantiable\n";
    }
    $sql.=";\n";
...
ScriptTypes.pl - Generates formal creation scripts for Oracle data types

=head1 SYNOPSIS

=over 4

=item B<ScriptTypes.pl> B<-i> db1 B<-u> user B<-p> passwd

Builds the creation scripts for all the types in the schema 'user' on
database instance db1.

=item B<ScriptTypes.pl> B<-i> db1 B<-u> user B<-p> passswd B<-m> schema1 B<-m> schema2

Builds the creation scripts only for all the types in the schemas 'schema1'
...
=item B<ScriptTypes.pl> B<-i> db1 B<-u> user B<-p> passwd B<-t> schema1.table1

Builds the creation scripts only for the type 'schema1.table1' on database
instance db1.

=item B<ScriptTypes.pl> B<-i> db1 B<-u> user B<-p> passwd -l db2

Builds the creation scripts for all the types in the DBLink's default schemas
of the remote database instance, which is accessed from this database instance
db1 via database link db2.

=item B<ScriptTypes.pl> B<-i> db1 B<-u> user B<-p> passwd B<-m> schema1,schema2 B<-l> db2

Builds the creation scripts for all the types in the schemas 'schema1'
and 'schema2' on a remote database instance that is accessed from this
Oracle instance db1 via the database link db2.

=item B<ScriptTypes.pl> B<--help>
...
B<ScriptTypes.pl> B<-u|--user> UserId B<-p|--password> Passwd [B<-i|--instance> Instance name]
